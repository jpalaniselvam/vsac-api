...
describe('UtilityClient - Unit Tests', () => {
  ...
  describe('getOidProgram - Input Validation', () => {
    ...
    it('should throw error for undefined OID', async () => {
      await expect(sdk.getOidProgram(undefined as any, 'Test Program')).rejects.toThrow(
        'OID is required'
      );
    });

    it('should throw error for null OID', async () => {
      await expect(sdk.getOidProgram(null as any, 'Test Program')).rejects.toThrow(
        'OID is required'
      );
    });

    it('should throw error for empty program name', async () => {
      await expect(sdk.getOidProgram('2.16.840.1.113762.1.4.1', '')).rejects.toThrow(
        'Program name is required'
      );
    });

    it('should throw error for undefined program name', async () => {
      await expect(sdk.getOidProgram('2.16.840.1.113762.1.4.1', undefined as any)).rejects.toThrow(
        'Program name is required'
      );
    });

    it('should throw error for null program name', async () => {
      await expect(sdk.getOidProgram('2.16.840.1.113762.1.4.1', null as any)).rejects.toThrow(
        'Program name is required'
      );
    });
  ...
  describe('getProgramLatestProfile - Input Validation', () => {
    ...
    it('should throw error for undefined program name', async () => {
      await expect(sdk.getProgramLatestProfile(undefined as any)).rejects.toThrow(
        'Program name is required'
      );
    });

    it('should throw error for null program name', async () => {
      await expect(sdk.getProgramLatestProfile(null as any)).rejects.toThrow(
        'Program name is required'
      );
    });
  });
